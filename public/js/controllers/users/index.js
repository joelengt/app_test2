			  ...
			  socket.emit('TrackRoom', idConnect)
				...
				$http({
					...
					var template = `<div class="infoTracking__containner">
														...
														<div class="more-line" id="more-line"><span class="icon-icon_agregar_poste"></span> Ver más</div>
													</div>`

					infoTracking.innerHTML = template
					$('.users__right--mapCanvas').append(infoTracking)

					$('#more-line').on('click', moreLines)

					console.log(res.data.tracking_list.track_info)
					var track = res.data.tracking_list.track_info

					if (track.length < 21) {
						$('#more-line').remove()
					}

					function addTracking(track, more){
						for(var g = 0; g <= track.length - 1; g++) {
							var el_coord =  track[g]
							...
							if(more){
								// $('#listInfoCoord').appendChild(coord)
								var last = $('[data-last="first-last"]')
								$(coord).insertAfter(last)
								path.unshift(arr_item)
							} else {
								if (g === 0) {
									coord.setAttribute('data-last', 'first-last')
								}
								$('#listInfoCoord').prepend(coord)
								path.push(arr_item)
							}
						}
						printDrawPolyline(path)
					}
					...
					function printDrawPolyline(path){
						map.drawPolyline({
							path: path,
							strokeColor: '#22b573',
							strokeOpacity: 0.6,
					...
					function moreLines(){
						...
						$http({
							method: 'GET',
							url: '/dashboard/usuarios/tracking/' + idConnect + '/draw/more'
						}).then(function(res){
							console.log(res)
							Loader.delete('.infoTracking__containner', 'tracking-loader-more')
							map.removePolylines()
							var listTracking = res.data.tracking_list.track_info
							addTracking(listTracking, true)
							map.removeMarkers()
							initRoute(path)
							lastRoute(path)
						}, function(err){
