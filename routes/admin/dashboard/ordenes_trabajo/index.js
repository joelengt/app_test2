...
function FormWorkOrder (app, io) {
                                       ...
                                       tipo_servicio === 'tipo_servicio_C') {
                                        ...
                                        BuildWorkFilters(orders_status_pendientes, function (err, orders_status_pendientes1) {
                                            ...
                                            BuildWorkFilters(orders_status_en_curso, function (err, orders_status_en_curso1) {
                                                ...
                                                new_orders_status_en_curso = orders_status_en_curso1
                        ...
                        Cliente.findById({'_id': service_id}, function (err, client_find1){
                            ...
                            if(req.files.hasOwnProperty('new_file_upload')) {
                                ...
                                if(FilesCover2.extension === 'jpg' && FilesCover2.mimetype === 'image_360') {
                                    ...
                                    var filePath = `${scope_path_system}/uploads/news/${FilesCover2.filename}`

                                    sharp(filePath)
                                      //.resize(300)
                                      .compressionLevel(5)
                                      .toFile(`${scope_path_system}/uploads/news/re_${FilesCover2.filename}`, function(err) {
                                        // output.jpg is a 300 pixels wide and 200 pixels high image
                                        // containing a scaled and cropped version of input.jpg
                                        if(err) {
                                            return res.status(500).json({
                                        ...
                                        FilesCover2.filename = `re_${FilesCover2.filename}`
    ...
    app.delete('/dashboard/ordenes_trabajo/:work_order_id/delete/:type_service/:service_id/item/:position_name', isLoggedIn, function (req, res) {
        if(req.user.permiso === users_type.onwers || 
           req.user.permiso === users_type.admins ||
           req.user.permiso === users_type.officers) {
    ...
    app.post('/dashboard/ordenes_trabajo/:work_order_id/change-status/:type_status', isLoggedIn, function (req, res) {
        if(req.user.permiso === users_type.onwers || 
           ...
           console.log(work_order_id);
           console.log('>>>> Evento Cambiar Estados <<<<')
           console.log(type_status)
            ...
            Work_Order.findById({'_id': work_order_id}, function (err, work_order_here) {
                ...
                //work_order_here === null -> No se encontro
                if(work_order_here === null) {

                    console.log('La orden de trabajo NO EXISTE')
                    res.status(403).json({
                        status: 'not_found',
                    ...
                    if (type_status === work_order_status.reprogramado) {
                        ...
                        console.log('Es de REPROGRAMADO, Va a aceptar o rechazar');
                        console.log(req.body.report_accept);
                        ...
                        if(req.body.report_accept === 'si') {
                            ...
                            Notificaciones.findOne({'work_order_id': work_order_id, 'type_notification': 'reporte'}, function (err, noti_to_new) {
                                if(err) {
                                    return res.status(500).json({
                                      status: 'error_server',
                                ...
                                console.log('Notificaicion encontrada')

                                console.log('DATOS DE LA Notificacion')
                                console.log(noti_to_new);
                                ...
                                work_order_here.save(function (err, work_order_saved) {
                                    ...
                                    new_work_orden1.save(function(err) {
                                        ...
                                        noti_up.status_lectura = config.card_status.read
                                        noti_up.type_answer = config.notification_type.type_answer.reporte.aceptada
                                        noti_up.message_copy = {
                                        ...
                                        Notificaciones.update({'_id': noti_to_new._id}, noti_up, function (err) {
                                            if(err) {
                                                return res.status(500).json({
                                                  status: 'error_server',
                                            ...
                                            noti2.save(function (err) {
                                                ...
                                                Work_Order.remove({'_id': work_order_here._id}, function (err) {
                                                    if(err) {
                                                        return res.status(500).json({
                                                            status: 'error_server',
                                                            error: err
                            ...
                            Notificaciones.findOne({'work_order_id': work_order_id, 'type_notification': 'reporte'}, function (err, noti_to_new) {
                               ...
                               // Guardando cambios
                                work_order_here.save(function (err, work_order_saved) {
                                    ...
                                    // Actualizando estado de lectura de la notificacion
                                    noti_up.status_lectura = config.card_status.read
                                    noti_up.type_answer =   config.notification_type.type_answer.reporte.rechazada
                                    noti_up.message_copy = {
                                    ...
                                    // Guardando cambios en la base de datos, para notifiacione
                                    Notificaciones.update({'_id': noti_to_new._id}, noti_up, function (err) {
                                        if(err) {
                                            return res.status(500).json({
                                              status: 'error_server',
                        ...
                        console.log('El parametro de respuesta No es Correcto: solo "reprogramado" o "cancelado"');
